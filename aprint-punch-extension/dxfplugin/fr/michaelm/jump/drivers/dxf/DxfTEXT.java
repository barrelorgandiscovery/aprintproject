    ...
    public static DxfGroup readEntity(RandomAccessFile raf, FeatureCollection entities) throws IOException {
        ...
        try {
            while (null != (group = DxfGroup.readGroup(raf)) && group.getCode()!=0) {
                ...
                else if (group.getCode()==1) feature.setAttribute("TEXT", group.getValue());
                else if (group.getCode()==40) feature.setAttribute("TEXT_HEIGHT", new Double(group.getDoubleValue()));
                else if (group.getCode()==7) feature.setAttribute("TEXT_STYLE", group.getValue());
                else {}
            }
