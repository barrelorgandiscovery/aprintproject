...
public class PunchController implements PositionPanelListener, Disposable {
	...
	private PositionPanel posPanel;

	private MachineControl machineControl;

	private MachineCommandStream machineCommandStream;

	private PunchStatisticCollector punchStatisticCollector = new PunchStatisticCollector(
			19);
	...
	private void updatePosPanel() {
		...
		posPanel.updateState(currentPos,allcmds, currentPos > 0 && !isRunning(),
				currentPos < allcmds
						&& !isRunning(), timeLeft, distanceLeft, bookDistanceLeft, bookMetersDone);
	...
	public void pausePlay() throws Exception {
		...
		if (machineCommandStream != null) {
			...
			machineCommandStream = new MachineCommandStream(machineControl,
					pl.getPunchPlan(), new StreamingProcessingListener() {
						...
						public void commandProcessed(final int index) {
							...
							if (index > 0) {
								...
								XYCommand p1 = punchPlan
										.getXYCommandAtIndex(index - 1);
								XYCommand p2 = punchPlan
										.getXYCommandAtIndex(index);

								if (p1 != null && p2 != null) {
									distance = Math.sqrt(Math.pow(p1.getX()
											- p2.getX(), 2)
								...
								if (p2 != null) {
									...
									bookDistanceLeft = String.format(
											"%1$6.3f m / %2$6.3f m", //$NON-NLS-1$
											(latestXYCommand.getX() - p2.getX() )/ 1000.0, 
											total/1000.0
											);
									bookMetersDone = String.format(
											"%1$6.3f m", //$NON-NLS-1$
											p2.getX()/ 1000.0 
											);
							...
							punchStatisticCollector
									.informPunchStopped(distance);
							...
							nboperationsleft = pl.getPunchPlan()
							...
							if (index % 10 == 0) {
								...
								timeLeft = formatDuration(Duration
										.ofMillis((long) (nboperationsleft
												* punchStatisticCollector
														.getMeanTimePerPunch() * 1000)));

							}
							
							PunchController.this.distanceLeft = String.format(
									"%1$6.3f m", //$NON-NLS-1$
