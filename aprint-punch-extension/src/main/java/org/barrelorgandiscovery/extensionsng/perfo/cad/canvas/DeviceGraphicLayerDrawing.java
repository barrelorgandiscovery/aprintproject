...
public class DeviceGraphicLayerDrawing extends DeviceDrawing {
	...
	private static Logger logger = Logger.getLogger(DeviceGraphicLayerDrawing.class);
	...
	protected void addObject(Geometry g) {
		...
		if (g instanceof LineString) {
			...
			if (coords.length == 2) {
				gl.add(new Line2D.Float((float) coords[0].x, (float) (scalewidth - coords[0].y), (float) coords[1].x,
						(float) (scalewidth - coords[1].y)));
				...
				if (coords.length > 0) {
					gp = new GeneralPath();
					gp.moveTo((float) coords[0].x, (float) (scalewidth - coords[0].y));
					for (int i = 1; i < coords.length; i++) {
						gp.lineTo((float) coords[i].x, (float) (scalewidth - coords[i].y));
					}
			...
			logger.error(
					"error in adding geometry " + g + ", this geometry is not supported in " + getClass().getName());
		}
