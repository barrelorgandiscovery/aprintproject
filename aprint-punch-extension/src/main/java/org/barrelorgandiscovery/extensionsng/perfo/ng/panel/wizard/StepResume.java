...
public class StepResume extends JPanel implements Step {
	...
	public StepResume(StepChooseMachine sm, VirtualBook vb, IPrefsStorage ps)
			throws Exception {
	...
	protected void initComponents() throws Exception {
		...
		punch.setIcon(new ImageIcon(PunchCommandPanel.class
				.getResource("smallpunch.png"))); //$NON-NLS-1$
	...
	public void activate(Serializable state, WizardStates allStepsStates,
		...
		StepPlanningState s = allStepsStates.getPreviousStateImplementing(this,
				StepPlanningState.class);
	...
	class ExportAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				if (currentPlan == null) {
					JMessageBox.showMessage(StepResume.this,
							Messages.getString("StepResume.14")); //$NON-NLS-1$
					return;
				}
				...
				fc
						.setFileFilter(new FileNameExtensionFilter(
								"GCode File", "gcode")); //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (f == JFileChooser.APPROVE_OPTION) {
					...
					// export plan
					PunchPlanIO.exportToGRBL(fileToSave, currentPlan);
					JMessageBox.showMessage(
							StepResume.this,
							Messages.getString("StepResume.15") //$NON-NLS-1$
									+ fileToSave
									+ Messages.getString("StepResume.16")); //$NON-NLS-1$
				}
	...
	class PunchFrameInternalFrame extends APrintNGInternalFrame {
		...
		public PunchFrameInternalFrame(IPrefsStorage prefsStorage,
				MachineControl machineControl) throws Exception {
			...
			PunchPlan planCopy = new PunchPlan(currentPlan);
			planCopy.getCommandsByRef().add(0, origin);
			PunchCommandPanel punchCommandPanel = new PunchCommandPanel(
					planCopy, vb, ps);
	...
	class PunchAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				AbstractMachineParameters machineParameters = sm
						.getMachineParameters();
				AbstractMachine machine = machineParameters
						.createAssociatedMachineInstance();

				MachineControl machineControl = machine.open(machineParameters);

				APrintNGInternalFrame aif = new PunchFrameInternalFrame(ps,
						machineControl);
				...
				logger.error(
						"error launching the punch, :" + ex.getMessage(), ex); //$NON-NLS-1$
