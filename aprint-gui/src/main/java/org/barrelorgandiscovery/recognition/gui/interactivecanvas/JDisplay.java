...
public class JDisplay extends JComponent implements CurrentToolChangedAware {
  ...
  public void panTo(Point2D centerPoint) throws Exception {
    ...
    t.concatenate(
        AffineTransform.getTranslateInstance(
            centerPoint.getX() - r.getX(), centerPoint.getY() - r.getY()));
  ...
  public void zoomTo(Rectangle2D.Double rectangle) {
    ...
    t.concatenate(
        AffineTransform.getTranslateInstance(
            -centerX + getWidth() / 2 / newFactor, -centerY + getHeight() / 2 / newFactor));
   ...
   * @throws Exception
   */
  public void scaleAndCenterOnScreenCoordinates(double factor, int screenx, int screeny)
      throws Exception {
    ...
    t.setToTranslation(
        getWidth() / 2 - uncorrectedDestination.getX(),
  ...
  public Rectangle2D.Double convertToOriginalImagePosition(Rectangle2D.Double rect)
      throws Exception {
    assert rect != null;
    ...
    return new Rectangle2D.Double(
        originTopleft.x,
        originTopleft.y,
        originBottomRight.x - originTopleft.x,
  ...
  public void setCurrentTool(Tool t) {
    if (currentTool != null) currentTool.unactivated();

    Tool oldTool = currentTool;

    currentTool = t;
    if (currentTool != null) t.activated();
    ...
    try {
      ...
      for (JLayer layer : layers) {
        ...
        if (extent == null) {
          ...
          if (layerExtent != null) Rectangle2D.union(layerExtent, extent, extent);
        }
      }
