 ...
 * 
 */
public class MoveAndCreatePointForMultipleLinesTool extends Tool {

	private static Logger logger = Logger.getLogger(MoveAndCreatePointForMultipleLinesTool.class);
	...
	private class CursorOnLine extends State {
		...
		public CursorOnLine(Point2D.Double projectedPoint, int shapelayerIndex, int index) {
		...
		public void mousePressed(MouseEvent e) {
			...
			try {
				...
				shapeLayer[shapelayerIndex].add(insertedPoint, index);

				ArrayList<Double> col = new ArrayList<Rectangle2D.Double>();
				col.add(insertedPoint);
				shapeLayer[shapelayerIndex].setSelected(col);
				changeState(new MoveState(e, shapelayerIndex));
	...
	private class CreateAndSnapState extends State {
		...
		private void callSnapBehaviour(MouseEvent e) {
			try {
				...
				for (int k = 0; k < shapeLayer.length; k++) {

					List<Double> g = shapeLayer[k].getGraphics();
					if (g.size() >= 2) {
						...
						for (int i = 0; i < g.size() - 1; i++) {
							...
							Point2D.Double doubleOrigin = new Point2D.Double(originPosition.getX(),
									originPosition.getY());
							...
							if (v.norme() < 5 * pixelsize) {
								...
								if (ov.angle(ot) < 1e-3 && ov.angle(ot) >= 0 && ot.norme() < ov.norme()) {
									...
									changeState(new CursorOnLine(intersectPoint, k, i + 1));
									return;
								}
							}
						}
	...
	private class MoveState extends State {
		...
		public void mouseDragged(MouseEvent e) {
			try {
				// move selection
				// show move
				Set<Double> currentSelection = shapeLayer[shapeLayerIndex].getSelected();
