...
public class ModelEditor extends JPanel {
	...
	private class MyMultiplicity extends mxMultiplicity {
		...
		@Override
		public String check(com.mxgraph.view.mxGraph graph, Object edge, Object source, Object target, int sourceOut,
				int targetIn) {
	 ...
	 * @param repository2
	 */
	public ModelEditor(ModelStepRegistry registry, Repository2 repository2, AsyncJobsManager asyncJobManager)
			throws Exception {
	 ...
	 */
	public ModelEditor(ModelStepRegistry registry, Repository2 repository2, AsyncJobsManager asyncJobManager,
			Map<String, Object> contextAmbiantObjects) throws Exception {
		...
		mxIEventListener undoHandler = new mxIEventListener() {
			public void invoke(Object source, mxEventObject evt) {
				List<mxUndoableChange> changes = ((mxUndoableEdit) evt.getProperty("edit")).getChanges(); //$NON-NLS-1$
		...
		if (rms != null) {
			for (Iterator iterator = rms.iterator(); iterator.hasNext();) {
				...
				try {
					...
					logger.error("error in creating modelstep cell :" + modelStep + ":" + ex.getMessage(), ex); //$NON-NLS-1$ //$NON-NLS-2$
				}
	...
	protected void replaceModelStep(mxCell cellToReplace, ModelStep step) throws Exception {
		...
		try {
			...
			mxGeometry adaptedGeometry = new mxGeometry(oldGeometry.getX(), oldGeometry.getY(), newGeometry.getWidth(),
	...
	private void reconnect(mxICell newCell, ToReconnect reconnect) {
		...
		Object[] newParam = mxGraphModel.filterCells(potentialChilds.toArray(new mxICell[potentialChilds.size()]),
				new Filter() {
					@Override
					public boolean filter(Object cell) {
						...
						if (!(c.getValue() instanceof Element))
							return false;

						Element e = (Element) c.getValue();
						String name = e.getAttribute("name"); //$NON-NLS-1$
						if (reconnect.removedCellParamName.equals(name))
							return true;
	...
	private mxICell findCellById(final String id) {
		...
		Collection<Object> res = mxGraphModel.filterDescendants(mxGraph.getModel(), new Filter() {
			@Override
			public boolean filter(Object cell) {
				if (!(cell instanceof mxICell)) {
					return false;
		...
		if (cell instanceof mxCell) {
			...
			if (newStyles.containsKey(id)) {
				...
				if (cellState.isError()) {
					...
					mxCellOverlay mxcellOverlay = new mxCellOverlay(
					...
					mxcellOverlay.addMouseListener(new MouseAdapter() {
						@Override
						public void mouseClicked(MouseEvent e) {
	...
	public static void main(String[] args) throws Exception {
		...
		final ModelEditor me = new ModelEditor(new ModelStepRegistry() {
			...
			public List<ModelStep> getRegisteredModelStepList() {
				...
				try {
					...
					ms.add(new TerminalParameterModelStep(true, new JavaType(MidiEventGroup.class), "Midi", "Midi", //$NON-NLS-1$ //$NON-NLS-2$
							null));

					ms.add(new TerminalParameterModelStep(false, new JavaType(VirtualBook.class), "VirtualBook", //$NON-NLS-1$
					...
					ms.add(new TerminalParameterModelStep(false, new JavaType(String.class), "String", "String", null)); //$NON-NLS-1$ //$NON-NLS-2$
					// try {
					//
		...
		undo.setAction(new AbstractAction("undo") { //$NON-NLS-1$
			public void actionPerformed(ActionEvent e) {
				me.undo();
			}
		});
		...
		redo.setAction(new AbstractAction("redo") { //$NON-NLS-1$
			public void actionPerformed(ActionEvent e) {
				me.redo();
			}
		});
		...
		tb.add(new JButton(me.bind("Zoom Actual", mxGraphActions.getZoomActualAction(), (String) null))); //$NON-NLS-1$

		f.getContentPane().add(tb, BorderLayout.NORTH);

		// add terminal for the model
		me.addTerminal(
				new TerminalParameterModelStep(true, new JavaType(VirtualBook.class), "virtualBook", "label", null)); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void addDefaultStyleSheet() {
		...
		Document docStyle = mxUtils.loadDocument(ModelEditor.class.getResource("default-style.xml").toString()); //$NON-NLS-1$
	...
	public void saveGraph(AbstractFileObject file) throws Exception {
		...
		OutputStream outputStream = VFSTools.transactionalWrite(file);
		try {
			...
			asyncJobsManager.submitAndExecuteJob(() -> {
				...
				public void jobError(Throwable ex) {
					...
					execInUIThread(() -> {
						...
						pw.flush();
						sw.flush();
						console.appendOutput(sw.getBuffer().toString(), null);
					});
				}
	...
	/**
	 * Edition des param�tres de configuration
	 *
	 * @param mxcell cellule edit�e
	 * @throws Exception
	 */
	...
	public Action bind(String name, final Action action, String iconUrl) {
		return new AbstractAction(name,
				(iconUrl != null) ? new ImageIcon(ModelEditor.class.getResource(iconUrl)) : null) {
			public void actionPerformed(ActionEvent e) {
