...
/**
 * Wizard component, maintain the wizard gui, maintain the current step state, maintain the
 * navigation logic
 *
 * @author pfreydiere
 */
public class Wizard extends JPanel implements Disposable {
  ...
  public Wizard(List<Step> steps, Serializable initialState) throws Exception {
    ...
    if (stepsContents == null) stepsContents = new WizardStates();

    initComponent();

    stepStatusChangedListener =
        new StepStatusChangedListener() {
  ...
  public void reinit(Serializable initialState) throws Exception {
    ...
    if (initialState == null) this.stepsContents = new WizardStates();
        ...
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            try {
              ...
              logger.error(
                  "error while going next :" + ex.getMessage(), //$NON-NLS-1$
                  ex);
            }
          }
        });
  ...
  private void moveToStep(int stepNo, boolean saveCurrentState) throws Exception {
    ...
    if (stepDetails == null) stepDetails = ""; //$NON-NLS-1$
