...
public class JInstrumentTileViewerPanel extends JPanel {
  ...
  public JInstrumentTileViewerPanel(
  ...
  protected void initComponent() throws Exception {
    InputStream formInputStream =
        JInstrumentTileViewerPanel.class.getResourceAsStream("instrumenttiledchooser.jfrm"); //$NON-NLS-1$
    ...
    choose.addActionListener(
        e -> {
          try {
            ...
            logger.error(
                "error while processing the instrument selection event handler :" + ex.getMessage(),//$NON-NLS-1$
                ex);
          }
        });
