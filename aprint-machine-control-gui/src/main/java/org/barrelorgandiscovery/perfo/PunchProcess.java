...
public class PunchProcess {
	...
	private void searchForFile(File folder, List<File> retrievedFiles) {
		...
		File[] elements = folder.listFiles(new FileFilter() {
			@Override
			public boolean accept(File pathname) {
				return pathname != null && pathname.isFile()
						&& (pathname.getName().endsWith(".punch") || pathname.getName().endsWith(".gcode"));
			}
		});
	 ...
	 * @throws Exception
	 */
	public File[] startPunch(PunchListener listener, File[] filesToPunch, final IPunchParameters punchParameters)
			throws Exception {
			...
			public void run() {
				...
				try {
					...
					for (Entry<File, PunchPlan> e : punchplan.entrySet()) {
						...
						backgroundThreadProcessPunchPlan(fileToProcess, punchPlan, punchParameters, isFirst, listener,
								finalCancelTracker);
	...
	private void backgroundThreadProcessPunchPlan(final File file, PunchPlan p, IPunchParameters punchParameters,
			boolean isFirstFile, PunchListener listener, ICancelTracker cancelTracker) throws Exception {
		...
		logger.debug("range visitor elements " + rv.getXmin() + "," + rv.getYmin() + " " + rv.getXmax() + ","
				+ rv.getYmax());
		...
		try {
			...
			open.setMachineControlListener(new MachineControlListener() {
				...
				@Override
				public void currentMachinePosition(String status, double mx,
						double my) {
					logger.debug("status position :" + mx + "," + my);
					if (listener != null) {
						SwingUtilities.invokeLater(() -> {
							listener.message(String.format("X %.2f Y %.2f ... - %s ", mx, my, status));
						});
					}
				}
