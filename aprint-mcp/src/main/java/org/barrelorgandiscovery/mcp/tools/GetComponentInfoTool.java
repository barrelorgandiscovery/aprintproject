...
/**
 * Tool for getting detailed information about a Swing component.
 * 
 * @author APrint Development Team
 */
public class GetComponentInfoTool {
	
	private static final Logger logger = Logger.getLogger(GetComponentInfoTool.class);
	...
	public static McpSchema.Tool createTool() {
		...
		return McpSchema.Tool.builder()
			...
			.description("Get detailed information about a Swing component by its path. Returns component type, name, text, visibility, bounds, properties, and child components.")
	...
	public static java.util.function.BiFunction<McpSyncServerExchange, CallToolRequest, CallToolResult> 
		...
		return (exchange, request) -> {
			...
			try {
				...
				logger.info("Getting component info: windowId=" + windowId + ", componentPath=" + componentPath);
				
				SwingComponentInfo info = context.getComponentInfo(windowId, componentPath);
				
				if (info == null) {
					Map<String, Object> response = new HashMap<>();
					response.put("found", false);
					response.put("message", "Component not found: " + componentPath);
				...
				logger.error("Error getting component info: " + e.getMessage(), e);
