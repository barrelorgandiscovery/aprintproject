...
/**
 * Tool for listing all open windows (VirtualBookFrames, consoles, etc.).
 * 
 * @author APrint Development Team
 */
public class ListAllWindowsTool {
	
	private static final Logger logger = Logger.getLogger(ListAllWindowsTool.class);
	...
	public static McpSchema.Tool createTool() {
		...
		return McpSchema.Tool.builder()
			...
			.description("List all open windows in the application (VirtualBookFrames, script consoles, etc.). " +
			       "Windows are ordered by most recent usage. Returns window information including type, ID, title, and relevant identifiers.")
	...
	public static java.util.function.BiFunction<McpSyncServerExchange, CallToolRequest, CallToolResult> 
		...
		return (exchange, request) -> {
			...
			try {
				...
				List<ActiveWindowInfo> windows = context.listAllWindows();
				
				Map<String, Object> response = new HashMap<>();
				response.put("windows", windows.stream().map(ActiveWindowInfo::toMap).toList());
				...
				logger.error("Error listing all windows: " + e.getMessage(), e);
