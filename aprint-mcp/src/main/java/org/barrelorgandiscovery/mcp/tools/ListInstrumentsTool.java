...
/**
 * Tool for listing all available instruments in the repository.
 * 
 * @author APrint Development Team
 */
public class ListInstrumentsTool {
	
	private static final Logger logger = Logger.getLogger(ListInstrumentsTool.class);
	...
	public static McpSchema.Tool createTool() {
		...
		return McpSchema.Tool.builder()
			...
			.description("List all available instruments in the repository. Returns an array of instrument names.")
	...
	public static java.util.function.BiFunction<McpSyncServerExchange, CallToolRequest, CallToolResult> 
		...
		return (exchange, request) -> {
			...
			try {
				...
				String[] instruments = context.listInstruments();
				
				Map<String, Object> response = new HashMap<>();
				response.put("instruments", Arrays.asList(instruments));
				response.put("count", instruments.length);
				...
				logger.error("Error listing instruments: " + e.getMessage(), e);
