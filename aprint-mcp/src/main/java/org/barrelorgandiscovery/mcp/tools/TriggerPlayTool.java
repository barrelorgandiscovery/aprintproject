 ...
 */
public class TriggerPlayTool {
	
	private static final Logger logger = Logger.getLogger(TriggerPlayTool.class);
	...
	public static McpSchema.Tool createTool() {
		...
		return McpSchema.Tool.builder()
			.name("trigger_play")
			.description("Start playing the currently active virtual book.")
	...
	public static java.util.function.BiFunction<McpSyncServerExchange, CallToolRequest, CallToolResult> 
		...
		return (exchange, request) -> {
			...
			try {
				...
				logger.error("Error starting playback: " + e.getMessage(), e);
				try {
					Map<String, Object> response = new HashMap<>();
