...
public class JOutputFolderChooser extends BasePanelStep {

	private static Logger logger = Logger.getLogger(JOutputFolderChooser.class);
	...
	public JOutputFolderChooser(Step parent, IPrefsStorage ps) throws Exception {
	...
	protected void initComponents() throws Exception {
		...
		btnfolder.addActionListener((e) -> {
			...
			JDirectoryChooser c;
			try {
				Cursor old = JOutputFolderChooser.this.getCursor();
				try {
					JOutputFolderChooser.this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
					...
					JOutputFolderChooser.this.setCursor(old);
				}
				int result = c.showOpenDialog(JOutputFolderChooser.this);
