...
public class ImageBookMergeModel implements Serializable {
	...
	public AffineTransform computeTransform(double factor) {
		...
		MathVect bv = getBookWidthVector();
		MathVect av = getAngleAndImageWidthVector();
		boolean revert = bv.vectorielZ(av) < 0;
		...
		t.preConcatenate(AffineTransform.getScaleInstance(1.0, revert? 1.0: -1.0));
	...
	public MathVect getAngleAndImageWidthVector() {
		return new MathVect(pointforAngleAndImageWidth.getX() - origin.getX(),
				pointforAngleAndImageWidth.getY() - origin.getY());
	}
	...
	public BufferedImage createSlice(BufferedImage current, int outImageWidth, int outImageHeight) {
		...
		BufferedImage newImage = new BufferedImage(outImageWidth, outImageHeight, BufferedImage.TYPE_INT_ARGB);
	...
	public void loadFrom(IPrefsStorage storage) {
		origin = new Point2D.Double(storage.getDoubleProperty("originx", 0), storage.getDoubleProperty("originy", 0));
